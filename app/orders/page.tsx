...
const orders = [
  {
    ...
    shipping: 'Nagpur, Maharashtra, India',
    items: [
      { name: 'Wireless Bluetooth Earbuds', qty: 1, price: 1999 },
      { name: 'Smart Watch Series 7', qty: 1, price: 2899 },
    ],
  },
  {
    id: 'ORD-10031',
    ...
    shipping: 'Pune, Maharashtra, India',
    items: [
      { name: 'Nikon Camera', qty: 1, price: 4999 },
      { name: 'Bluetooth soundbox Series 7', qty: 1, price: 4999 },
    ],
  },
];
                  ...
                  <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_320px]">
                    <div className="space-y-4">
                      <div className="rounded-3xl bg-purple/5 border border-purple/10 p-6">
                        ...
                        <SubText>{order.shipping}</SubText>
                      </div>
                      <div className="rounded-3xl bg-purple/5 border border-purple/10 p-6">
                      ...
                      <div className="rounded-3xl bg-purple/5 border border-purple/10 p-6">
                        ...
                        <div className="space-y-3 mt-3">
                          {order.items.map((item) => (
                            <div key={item.name} className="flex justify-between gap-4 rounded-3xl bg-white border border-slate-100 p-4">
                              <div>
                                <p className="font-semibold text-slate-900">{item.name}</p>
                                <p className="text-sm text-slate-500">Qty: {item.qty}</p>
                              </div>
                              <p className="font-semibold text-slate-900"><PriceFormatter amount={item.price * item.qty} /></p>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="rounded-3xl bg-slate-50 border border-slate-200 p-6">
                      ...
                      <div className="mt-4 space-y-3 text-sm text-slate-600">
                        <div className="flex justify-between">
                          <span>Subtotal</span>
                          <span><PriceFormatter amount={order.total - 100} /></span>
                        </div>
                        <div className="flex justify-between">
                          <span>Shipping</span>
                          <span><PriceFormatter amount={100} /></span>
                        </div>
