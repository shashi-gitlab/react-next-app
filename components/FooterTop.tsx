...
const data:ContactItemData[] = [
  {
    ...
    icon:(
        <MapPin className='h-6 w-6 group-hover:text-secondary-color transition-colors' />
    )
  },
  {
    title: "Call Us",
    ...
    <div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8 border-b'>
        {data.map((item,index) => (
            <div key={index} className='flex items-center gap-3 group hover:bg-gray-50 py-4 transition-colors hoverEffect'>
                <span className='text-secondary-color /70'>{item.icon}</span>
                <div>
                    <h3 className='font-semibold text-purple/70 group-hover:text-primary-color transition-colors hoverEffect'>{item.title}</h3>
